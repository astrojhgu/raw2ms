...
public:
  visb_by_baseline_source(const std::vector<std::string>& antenna_names,
    ...
    :delay_vec(delay_vec1),raw_data_source(antenna_names.size()*(antenna_names.size()+1)/2),chlimits(_chlimits),time_file(data_path+"/time-0-"+date+".txt"),current_time(0),base_band_idx(0)
  {
...
int main (int argc, char** argv)
{
  if(argc<7)
    {
      std::cerr<<"Usage:"<<argv[0]<<" <antenna table> <delay data> <out name> <date> <input path> <ch1:ch2> [ch3:ch4] [ch5:ch6]..."<<std::endl;
      return -1;
    }
