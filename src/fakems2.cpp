...
class visb_by_baseline_source : public raw_data_source
{
  ...
  public:
    ...
    : raw_data_source (antenna_names.size () * (antenna_names.size () + 1) / 2), chlimit (_chlimit),
      time_file (time_file_name), current_time (0), nantennas (antenna_names.size ())
    {
...
int main (int argc, char **argv)
{
    if (argc != 8)
        {
            std::cerr << "Usage:" << argv[0]
                      << " <antenna table> <out name> <time file> <ra> <dec> <chbeg> <chend>" << std::endl;
            return -1;
        }
    ...
    visb_by_baseline_source vbs (antNameVec, time_file_name, chlimit);
