...
class visb_by_baseline_source : public raw_data_source
{
  ...
  public:
    ...
    : gain_vec (_gain_vec), time_step (_time_step),
      raw_data_source (antenna_names.size () * (antenna_names.size () + 1) / 2),
      chlimits (_chlimits), time_file (data_path + "/time-0-" + date + ".txt"), current_time (0),
      current_idx (0), current_time_step_id (0), base_band_idx (0)
    {
                ...
                for (int j = i; j < nantennas; ++j)
                    {
                        ...
                        data_path + "/" + antenna_names.at (i) + antenna_names.at (j) + "-0-" + date + ".bin")));
        ...
        for (int i = 0; i < files.size (); ++i)
            {
                ...
                MBaseline bl (MVBaseline (MVPosition (blx, bly, blz)), MBaseline::ITRF);
                ...
                for (int j = 0; j < chlimits.size (); ++j)
                    {
                        ...
                        for (int k = ch_lower; k != ch_upper; ++k)
                            {
                                std::complex<float> gg =
                                gain_vec.at (k).at (current_time_step_id).at (antenna_pair.second) *
                                std::conj (
                                gain_vec.at (k).at (current_time_step_id).at (antenna_pair.first));
                                ...
                                if (uv_lambda > max_uv)
                                    {
                                        std::cerr << "ants:" << antenna_pair.first << " "
                                                  << antenna_pair.second << endl;
...
int main (int argc, char **argv)
{
    if (argc < 8)
        {
            std::cerr << "Usage:" << argv[0] << " <antenna table> <gain_prefix> <step file> <out name> <date> <input path> <ch1:ch2> [ch3:ch4] [ch5:ch6]..."
                      << std::endl;
            return -1;
        }
    mxr = 0;
                    ...
                    for (int k = 0; k < nantennas; ++k)
                        {
                            ...
                            gain.at (k) =
                            std::complex<float> (ampl * std::cos (phase), ampl * std::sin (phase));
                        }
                    if (!gain_vec[i].empty ())
                        {
    ...
    visb_by_baseline_source vbs (antNameVec, gain_vec, time_step_vec, input_path, date, chlimits);
